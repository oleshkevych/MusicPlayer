package com.example.vov4ik.musicplayer.data.local.db;
...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private final static String COLUMN_FOLDER = "Folder";
    private final static String COLUMN_TITLE = "Title";
    private final static String COLUMN_ALBUM = "Album";
    private final static String COLUMN_ARTIST = "Artist";
    private final static String COLUMN_MAIN_FOLDER = "MainFolder";
    private final static String COLUMN_TRACK_NUMBER = "TrackNumber";
...
//        try {
            ...
            ArrayList<String> currentSubItems = new ArrayList<>();

            while (cursor.moveToNext()) {
//                Log.d(Constants.LOG_TAG, "Fe \n" + cursor.getCount());
//                Log.d(Constants.LOG_TAG, "Fe \n" + cursor.getColumnCount());
                if (cursor.getString(0) != null && !cursor.getString(0).equals(currentItemName)) {
            ...
            Log.i(Constants.LOG_TAG, "Data fetched and processed in " + (endTime - startTime) + " ms.");
//        } catch (Exception e) {
//            Log.e(Constants.LOG_TAG, "Data fetching exception " + e);
//        }
        return items;
    }
    ...
    public List<MusicFile> getAllSongs() {
        return getMusicFilesForSearch();
    }
    ...
    public List<MusicFile> getLastPlayList() {
        createDbIfNotExist(mDatabase);
        ...
        do {
            MusicFile musicFile = new MusicFile(
                    ...
                    cursor.getString(cursor.getColumnIndex(COLUMN_MAIN_FOLDER)),
                    cursor.getString(cursor.getColumnIndex(COLUMN_TRACK_NUMBER))
            );
            list.add(musicFile);
        } while (cursor.moveToNext());
        cursor.close();
    ...
    public void setLastPlayList(List<MusicFile> list) {
        createDbIfNotExist(mDatabase);
        Log.d("Test", "DATABASE is writing current trek list");
        mDatabase.delete(PLAY_SERVICES, null, null);
        mDatabase.execSQL(DROP_TABLE + PLAY_SERVICES);
        mDatabase.execSQL(CREATE_TABLE + PLAY_SERVICES + " (" +
                ID + " INTEGER PRIMARY KEY, " + COLUMN_PATH + " TEXT, " + COLUMN_FOLDER + " TEXT, "
                + COLUMN_TITLE + " TEXT, " + COLUMN_ALBUM + " TEXT, " + COLUMN_ARTIST + " TEXT, "
                + COLUMN_MAIN_FOLDER + " TEXT, " + COLUMN_TRACK_NUMBER + " TEXT);");
        for (int i = 0; i < list.size(); i++) {
            ContentValues values = new ContentValues();
            values.put(COLUMN_PATH, list.get(i).getPath());
            values.put(COLUMN_FOLDER, list.get(i).getFolder());
            values.put(COLUMN_TITLE, list.get(i).getTitle());
            values.put(COLUMN_ALBUM, list.get(i).getAlbum());
            values.put(COLUMN_ARTIST, list.get(i).getArtist());
            values.put(COLUMN_MAIN_FOLDER, list.get(i).getMainFolder());
            values.put(COLUMN_TRACK_NUMBER, list.get(i).getTrackNumber());
            mDatabase.insert(
                    PLAY_SERVICES,
                    null,
                    values);
    ...
    public void setPlaylistAttributes(int time, int number, boolean shuffle) {
        ...
        mDatabase.execSQL(DROP_TABLE + PLAY_SERVICE_ATTRIBUTES);
        mDatabase.execSQL(CREATE_TABLE + PLAY_SERVICE_ATTRIBUTES + " (" +
